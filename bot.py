...
async def pickup_date(callback: CallbackQuery, state: FSMContext):
    d = date.fromisoformat(callback.data.split(":")[1])
...
async def pickup_time(callback: CallbackQuery, state: FSMContext):
    await state.update_data(pickup_time=callback.data.split(":")[1])
...
async def return_time(callback: CallbackQuery, state: FSMContext):
    ...
    return_t = datetime.strptime(callback.data.split(":")[1], "%H:%M").time()
    ...
    await state.update_data(
        return_time=callback.data.split(":")[1],
        pickup_at=pickup_at,
        return_at=return_at,
...
async def admin_new_pickup_time(callback: CallbackQuery, state: FSMContext):
    await state.update_data(new_pickup_time=callback.data.split(":")[1])
